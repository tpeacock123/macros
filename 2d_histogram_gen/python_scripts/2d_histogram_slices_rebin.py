...
def moving_average(window, hist):
    ...
    averageHisto = ROOT.TH1D("averagehisto_{}".format(name),"averagehisto_{}".format(name),500,0.0,2000.0)
...
legend1 = ROOT.TLegend(0.67,0.12,0.89,0.50) # sets up first legend
legend1.SetHeader("energy value","c") 
...
for i in range(0,50): 
    energy = 10*i # increment the 
    plist.append(f.ProjectionX("p{}".format(energy), 10*i, 2000, "o"))
    plist[i].Divide(p0)
    ...
    legend1.AddEntry(averageList[i],"fermi momentum {}".format(energy),"l")
legend1.Draw()
...
Ke_vs_FermiMomentum = ROOT.TH2D("Weight_on_x-sec_pp","Weight_on_x-sec",500,0.0,2000.0,49,0,500)
...
outputFile = ROOT.TFile.Open("incomingKE_vs_outgoingKE_slices_new_rebin.root", "recreate")
canv.Write()
canv2.Write()
canv3.Write()
outputFile.Close()

outputFile2 = ROOT.TFile.Open("neutinputFile_pp_rebin.root", "recreate")
